...
namespace wjkYouTupe
{
    class DbLayer
    {
        ...
        /// If the record can't be found in the database it will be inserted. If the value is different from the database it will be updated.
        /// </summary>
        /// <param name="value">The text of the control by reference.</param>
        /// <param name="_key">The name of the control.</param>
        /// <returns>Returns true it the record was found, inserted or updated./</returns>
        public bool GetSingleTranslation(ref string value, string _key)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(_value) && tLang == Fallback)
                {
                    value = tValue;
                    return InsertInDB(tForm, tLang, tKey, tValue);
                }
                else if (_value != value && tLang == Fallback)
                {
                    value = tValue;
                    return UpdateDB(tForm, tLang, tKey, tValue);
                }
        ...
        public bool UpdateDB(string tForm, string tLang, string tKey, string tValue)
        {
            SqliteCommand cmd;
            cmd = conn.CreateCommand();
            try
            {
                ...
                cmd.Parameters.AddWithValue("@tKey", tKey);
                cmd.Parameters.AddWithValue("@tValue", tValue);
                cmd.CommandText = "UPATE Translate SET Form=@tForm, Lang=@tLang, Key=@tKey, Value=@tValue";
                if (conn.State == System.Data.ConnectionState.Closed) { conn.Open(); }
                var result = cmd.ExecuteNonQuery();
                return (result > 0);
            }
